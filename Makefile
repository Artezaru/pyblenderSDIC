...
latexpdf:
	...
	cd $(BUILDDIR)/latex && pdflatex pyblenderSDIC.tex && pdflatex pyblenderSDIC.tex
...
app:
	echo "from pyblenderSDIC.__main__ import __main_gui__" > run_gui.py
	echo "__main_gui__()" >> run_gui.py
	pyinstaller --name pyblenderSDIC --onefile --windowed run_gui.py
	rm run_gui.py
	rm -rf build
	rm run_gui.spec
